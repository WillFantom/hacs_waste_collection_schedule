...
API_URLS = {
    "BASE": "https://lcc-wrp.whitespacews.com"
}
ICON_MAP = {
    ...
    def fetch(self):
        ...
        for i in range(0, len(services_sub), 3):
            try:
                ...
                _LOGGER.info(f"Skipped {services_sub[i + 1].text.strip()} as it does not match time format")
                continue
            bin_type = BeautifulSoup(services_sub[i + 2].text, features="lxml").find("p")
            print(bin_type.text.strip().removesuffix(" Collection Service"))
            entries.append(
                Collection(
                    date=dt,
                    t=bin_type.text.strip().removesuffix(" Collection Service"),
                    icon=ICON_MAP[bin_type.text.strip().removesuffix(" Collection Service")],
                )
            )
